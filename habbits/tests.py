...
class HabbitModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpass', telegram_id='12345')
    ...
    def test_related_habit_and_reward_text_both_set(self):
        related_habit = Habbit.objects.create(
            place='Park', time=timezone.now(), action='Walk', is_rewarding=True,
            periodicity_days=1, duration_seconds=60, is_public=False, user=self.user
        )
        with self.assertRaises(ValidationError):
            habit = Habbit(
                place='Home', time=timezone.now(), action='Exercise', is_rewarding=False,
                related_habit=related_habit, reward_text='Watch TV',
                periodicity_days=1, duration_seconds=60, is_public=False, user=self.user
            )
            habit.full_clean()
    ...
    def test_periodicity_days_validation(self):
        with self.assertRaises(ValidationError):
            habit = Habbit(
                place='Home', time=timezone.now(), action='Exercise', is_rewarding=False,
                reward_text='Watch TV', periodicity_days=0, duration_seconds=60,
    ...
    def test_duration_seconds_validation(self):
        with self.assertRaises(ValidationError):
            habit = Habbit(
                place='Home', time=timezone.now(), action='Exercise', is_rewarding=False,
                reward_text='Watch TV', periodicity_days=1, duration_seconds=121,
                is_public=False, user=self.user
            )
            habit.full_clean()
...
class PublicHabbitListAPIViewTest(APITestCase):
    def setUp(self):
        ...
        self.public_habit = Habbit.objects.create(
            place='Park', time=timezone.now(), action='Jogging', is_rewarding=False,
            reward_text='Ice cream', periodicity_days=1, duration_seconds=60,
            is_public=True, user=self.user
        )
        self.private_habit = Habbit.objects.create(
            place='Home', time=timezone.now(), action='Reading', is_rewarding=False,
            reward_text='Relax', periodicity_days=1, duration_seconds=60,
            is_public=False, user=self.user
        )
...
class HabbitViewSetTest(APITestCase):
    def setUp(self):
        self.client = APIClient()
        self.user1 = User.objects.create_user(username='user1', password='pass1', telegram_id='111')
        self.user2 = User.objects.create_user(username='user2', password='pass2', telegram_id='222')
        self.habit1 = Habbit.objects.create(
            place='Home', time=timezone.now(), action='Exercise', is_rewarding=False,
            reward_text='Watch TV', periodicity_days=1, duration_seconds=60,
            is_public=False, user=self.user1
        )
        self.habit2 = Habbit.objects.create(
            place='Office', time=timezone.now(), action='Work', is_rewarding=False,
            reward_text='Coffee', periodicity_days=1, duration_seconds=60,
            is_public=False, user=self.user2
        )
    ...
    def test_create_habit(self):
        self.client.force_authenticate(user=self.user1)
        data = {
            'place': 'Gym', 'time': timezone.now().isoformat(), 'action': 'Workout',
            'is_rewarding': False, 'reward_text': 'Protein shake', 'periodicity_days': 1,
            'duration_seconds': 90, 'is_public': False
        }
    ...
    def test_pagination(self):
        ...
        for i in range(6):
            Habbit.objects.create(
                place=f'Place {i}', time=timezone.now(), action=f'Action {i}',
                is_rewarding=False, reward_text='Reward', periodicity_days=1,
                duration_seconds=60, is_public=False, user=self.user1
            )
        response = self.client.get('/habbits/')
        self.assertEqual(response.status_code, 200)
...
class SendNoticesTaskTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpass', telegram_id='12345')
        now = timezone.datetime(2023, 1, 1, 12, 0, 0, tzinfo=timezone.get_current_timezone())
        self.habit = Habbit.objects.create(
            place='Home', time=now, action='Exercise', is_rewarding=False,
            reward_text='Watch TV', periodicity_days=1, duration_seconds=60,
            is_public=False, user=self.user
        )
